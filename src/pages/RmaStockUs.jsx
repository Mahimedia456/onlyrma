  ...
  // CSV header mapping for US import
  const HUMAN_TO_CANON_US = {
    ...
    "A Stock - Received": "a_stock_received",
  ...
  export default function RmaStockUs() {
    ...
    // 1) Base device list from existing stock API
    useEffect(() => {
      (async () => {
        try {
    ...
    useEffect(() => {
      (async () => {
        try {
          ...
          setDevices((prev) =>
            Array.from(new Set([...(prev || []), ...fromEntries])).sort()
          );
        } catch {
          // ignore
        }
    ...
    useEffect(() => {
      (async () => {
        ...
        try {
          ...
          const res = await fetch(apiUrl(`/rma/us/stock?${qs.toString()}`), {
            credentials: "include",
          });
    ...
    const filteredRows = useMemo(() => {
      ...
      if (search.trim()) {
        ...
        list = list.filter((r) => (r.device_name || "").toLowerCase().includes(q));
      }
      return list;
    }, [rows, search]);
    ...
    async function saveNew() {
      ...
      const r = await fetch(apiUrl(`/rma/us/stock?${qs.toString()}`), {
        credentials: "include",
      });
    ...
    // CSV EXPORT
    function exportCSV() {
      if (!filteredRows.length) return alert("No rows to export for current filters.");
    ...
    async function handleImportFile(e) {
      ...
      try {
        if (!month) return alert("Select a month before importing.");
        ...
        const items = records
          .map((row) => {
            const device_name = String(row["Device Name"] || "").trim();
            if (!device_name) return null;

            const obj = { month, device_name, notes: "" };

            for (const [human, canon] of Object.entries(HUMAN_TO_CANON_US)) {
        ...
        if (!items.length) {
          setImportMsg("No valid rows (Device Name missing).");
          setImporting(false);
          return;
        }
        ...
        setImportMsg(`Import finished: ${ok} success, ${failed} failed.`);
        ...
        console.error(err);
        setImportMsg(`Import error: ${err.message || err}`);
      } finally {
        setImporting(false);
      }
    }
    ...
    if (error)
      return (
        <PageShell>
          <div className="p-6 text-red-600">Session error: {error}</div>
        </PageShell>
      );
      ...
      <PageShell>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-full sm:w-64">
            <label className="block text-xs font-medium mb-1">Search (Device Name)</label>
            <input
              className="border rounded px-3 py-2 w-full"
          ...
          <Quick
            ...
            onChange={(v) => setForm((f) => ({ ...f, d_stock_received: v }))}
          />
          ...
          <Quick
            ...
            onChange={(v) => setForm((f) => ({ ...f, b_stock_received: v }))}
          />
          ...
          <Quick
            ...
            onChange={(v) => setForm((f) => ({ ...f, new_stock_sent: v }))}
          />
          ...
          <Quick
            ...
            onChange={(v) => setForm((f) => ({ ...f, rma_bstock_rstock_sent: v }))}
          />

          <Quick
            number
            label="A Stock - Received"
            value={form.a_stock_received}
            onChange={(v) => setForm((f) => ({ ...f, a_stock_received: v }))}
          />
          ...
          <Quick
            ...
            onChange={(v) => setForm((f) => ({ ...f, awaiting_delivery_from_user: v }))}
          />

          <Quick
            number
            label="Receive Only"
            value={form.receive_only}
            onChange={(v) => setForm((f) => ({ ...f, receive_only: v }))}
          />
          ...
          <Quick
            ...
            onChange={(v) => setForm((f) => ({ ...f, awaiting_return_from_rush: v }))}
          />
                    ...
                    <Td>
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const res = await fetch(apiUrl(`/rma/us/stock/${r.id}`), {
                            method: "DELETE",
                            credentials: "include",
                          });
        ...
        <div className="flex flex-wrap justify-between items-center text-xs text-gray-600 gap-3 mt-4">
          <div>
            Showing {pageRows.length ? startIndex + 1 : 0} – {startIndex + pageRows.length} of{" "}
            {total}
          </div>
